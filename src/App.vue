...
const b30 = computed(() => [...scores.value].sort((a, b) => b.rating - a.rating).slice(0, 30))
const totalRating = computed(() =>
    (
        Math.floor(b30.value.reduce((sum, s) => sum + Math.round(s.rating * 100), 0) / 30) / 100
    ).toFixed(2),
)
const filtered = computed(() =>
    scores.value.filter(
        (s) =>
            `${s.ja} ${s.en} ${s.artist}`.toLowerCase().includes(query.value.toLowerCase()) &&
...
const completed = computed(
    () => jobs.value.filter((j) => j.status === 'success' || j.status === 'failed').length,
)
        ...
        if (!job) {
            if (!failures.value.length) {
                ...
                timer = setTimeout(() => {
                    ...
                    const duration = motionTiming.successHold + (reducedMotion
                        ? 0
                        : motionTiming.successCircle + motionTiming.successCheck)
                    timer = setTimeout(() => {
...
function simulate(withErrors = false) {
    startImport([
        { name: 'FP_result_001.png' },
        {
            ...
            error: withErrors ? '未匹配到曲目。请使用完整、清晰的成绩截图，或手动新增该成绩。' : '',
        },
        {
            name: 'FP_result_003.png',
            error: withErrors ? '无法解析 Score：分数区域不完整。请重新截取完整成绩画面。' : '',
        },
    ])
}
...
const currentChart = computed(
    () => editing.value ?? chartOptions.value.find((s) => s.difficulty === editorDifficulty.value)!,
)
...
const liveRating = computed(() =>
    validScore.value ? getChartRating(Number(inputScore.value), chartBase.value).toFixed(2) : '—',
)
    ...
    <div class="app-shell">
        <aside class="sidebar stack">
            ...
            <nav class="stack gap-2">
                <a
                    ...
                    ><component :is="n.icon" :size="19" /><span>{{ t(`nav.${n.id}`) }}</span
            ...
            <div class="sidebar-bottom">
                <div class="local-note">
                    <ShieldCheck :size="20" /><strong>留在本地，专注节奏。</strong>
                    <p>正式版将在浏览器本地处理截图，<br />只保存你的结构化成绩。</p>
        ...
        <div class="workspace">
            <header class="topbar between">
                ...
                <div class="row gap-4">
                    <span class="preview-pill"><span class="tiny-dot"></span> 界面预览</span
                    ><button
                        class="icon-button"
            ...
            <main>
                <div class="page-heading between">
                    <div>
                        ...
                        <h1>
                            {{ page === 'home' ? '让每一份成绩，都有迹可循。' : t(`nav.${page}`) }}
                        </h1>
                        <p>
                            {{
                                page === 'home'
                    ...
                    <button v-if="page === 'scores'" class="button primary" @click="openEditor()">
                        <Plus :size="17" /> 手动新增
                    </button>
                    <div v-if="page === 'home' || page === 'b30'" class="demo-label">
                ...
                <template v-if="page === 'home' || page === 'b30'">
                    <section class="stats-grid">
                        <article class="rating-card">
                            ...
                            <div class="big-rating">{{ totalRating }}<span>RT</span></div>
                            <div class="metric-bottom">每一份热爱，都在这里累积 <span>↗</span></div>
                        ...
                        <article class="stat-card">
                            ...
                            <div class="stat-value">{{ b30.length }} <span>/ 30</span></div>
                        ...
                        <article class="stat-card">
                            <div class="between">
                                <span>B30 Floor</span
                                ><span class="stat-icon amber"><ArrowDownToLine :size="18" /></span>
                            </div>
                        ...
                        <article class="stat-card">
                            <div class="between">
                                <span>已收录成绩</span
                                ><span class="stat-icon blue"><Disc3 :size="18" /></span>
                            </div>
                            <div class="stat-value">{{ scores.length }} <span>谱面</span></div>
                ...
                <template v-if="page === 'home'">
                    <section class="import-grid">
                        <article class="panel import-panel">
                            <div class="section-title between">
                                ...
                                <span class="micro-label">SCREENSHOT IMPORT</span>
                            </div>
                            <div
                                ...
                                <div class="upload-illustration">
                                    ...
                                    <div class="image-sheet">
                                        <FileImage :size="33" :stroke-width="1.3" /><span
                                            class="plus-bubble"
                                            >+</span
                                        >
                                    </div>
                            ...
                            <div class="import-foot between">
                                ...
                                <button class="text-link" @click="simulate(true)">
                                    演示识别错误
                                </button>
                            </div>
                        </article>
                    ...
                    <section class="panel">
                        <div class="section-title between">
                            <div class="row gap-2">
                                <h2>成绩一览</h2>
                                <span class="count-badge">{{ scores.length }}</span
                ...
                <template v-if="page === 'b30'">
                    <section class="panel">
                        <div class="section-title between">
                            <div class="row gap-2">
                                <Trophy :size="18" />
                                <h2>Best 30</h2>
                                <span class="count-badge">{{ b30.length }} 谱面</span>
                            </div>
                    ...
                    <p class="rating-explainer">
                        <CircleHelp :size="15" /> 总 RT 为前 30 张谱面 Rating 之和 ÷ 30；不足 30
                        张时按 0 补足。相同曲目的不同谱面可分别入选。
                    </p>
                </template>
                        ...
                        <div class="setting-row">
                            ...
                            <div class="theme-options">
                                <button
                                    ...
                                    <Check v-if="theme === option.id" :size="13" />
                                </button>
                            </div>
                        </div>
                        <div class="setting-row">
            ...
            <form @submit.prevent="previewSave">
                ...
                <div class="song-picker">
                    ...
                    <div v-if="!editing" class="song-options" aria-label="曲目搜索结果">
                        <button
                            ...
                            <span
                                >{{ song[songLanguage] }}<small>{{ song.artist }}</small></span
                            ><Check v-if="selectedSong === song.id" :size="16" />
                        </button>
                ...
                <fieldset class="chart-picker" :disabled="!!editing">
                    ...
                    <div class="chart-buttons">
                        <button
                            ...
                            :key="chart.id"
                            type="button"
                            :aria-pressed="editorDifficulty === chart.difficulty"
                            :class="{ selected: editorDifficulty === chart.difficulty }"
                ...
                <div class="live-metrics" aria-live="polite">
                    ...
                    <div>
                        ...
                        ><strong>{{ chartBase.toFixed(1) }} <span>→</span> {{ liveRating }}</strong>
                    </div>
                </div>
                <p v-if="!validScore" class="error">请输入 0～1,050,000 范围内的整数。</p>
                <p v-if="lowered" class="warning">
                    将调低「{{ editing?.[songLanguage] }}」的成绩，正式保存时 Rank 与 Rating
                    会重新计算。
                </p>
                <div class="form-note">
                ...
                <div class="dialog-actions">
                    <button type="button" class="button secondary" @click="dialog?.close()">
                        取消</button
                    ><button class="button primary" :disabled="!validScore">预览保存</button>
                </div>
            </form>
        </dialog>
        <dialog
            ...
            <div v-if="importPhase !== 'processing'" class="import-success" role="status">
                <AnimatedSuccess />
            ...
            <div class="between">
                <div>
                    ...
                    <h2 id="import-title">
                        {{
                            importHasErrors ? '部分图片识别失败' : '正在录入成绩'
                        }}
                    </h2>
                </div>
            </div>
            ...
            <div class="between import-counter" aria-live="polite">
                ...
                <span v-else>{{ completed }} / {{ jobs.length }} 已处理</span>
            </div>
        ...
        <dialog ref="deleteDialog" class="editor-dialog">
            ...
            <div class="dialog-actions">
                <button class="button secondary" @click="deleteDialog?.close()">保留成绩</button>
                <button
                    class="button primary"
                    @click="
                        (deleteDialog?.close(), notify('删除确认流程演示完成，示例成绩保持不变。'))
                    ">
                    预览删除
                </button>
            </div>
