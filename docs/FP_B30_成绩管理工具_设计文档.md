...
- 手动新增重复谱面报错，提示编辑已有成绩，不静默覆盖。编辑接口只接受记录 ID、Score 和降分确认，不接受歌曲或谱面变更。
- 降分显示曲名和警告，须勾选确认；仓储再次检查确认参数。OCR 使用事务比较分数，只更新更高分，支持并发写入且不会产生重复记录。
- source 表示最近一次实际修改分数的方式。createdAt 首次写入后不变；分数实际改变才更新 updatedAt/source；相同分数或更低 OCR 输入完全不更新记录。版本重算不改变时间或来源。
...
`yarn test` 验证 Rank／Rating 边界、整库 ID 与谱面关联、双语及别名搜索、非法分数拒绝、CRUD 和重开读取、重复新增、并发 OCR 最高分、降分确认、来源／时间语义及版本重算回滚。`yarn build` 执行 Vue／TypeScript 检查及生产构建。Edge 独立浏览器上下文已实测新增、刷新保留、编辑锁谱、降分确认、重复新增报错及删除；检查 390px 手机宽度布局。截图 OCR 仍是明确标记的流程演示，阶段 4、5 待实现。
