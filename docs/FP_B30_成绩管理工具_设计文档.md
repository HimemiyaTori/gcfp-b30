...
| --- | --- |
| 曲目 | 当前显示语言的曲名 |
| 难度 | 难度 → 等级 → 模式 → 当前显示语言的曲名 |
| 等级 | 等级 → 难度 → 模式 → 当前显示语言的曲名 |
| Score | Score → 等级 → 难度 → 模式 → 当前显示语言的曲名 |
| Rank | Rank → Score → 等级 → 难度 → 模式 → 当前显示语言的曲名 |
| Rating | Rating → Score → 等级 → 难度 → 模式 → 当前显示语言的曲名 |

每级相同才比较下一级，升降序应用于整条比较链。所有比较项都相同时保留原有顺序，ID 不参与排序。默认按 Score 降序；切换到曲目排序时默认升序，切换到其他表头时默认降序，再次点击同一表头切换方向。
