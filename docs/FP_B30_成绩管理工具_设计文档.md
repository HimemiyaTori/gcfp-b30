...
- 编辑时显示已有曲目，模式与难度按钮锁定，仅可修改 Score。
...
原 T05 已关闭：`en` 字段和 `vocal` 字段保证存在；第 5.2、6.2 节已统一为必填模型。为便于追踪，其他原编号保持不变。

| 编号 | 对应章节          | 需要补齐的规格                                                                  | 确定时机                       | 应落盘的结果                                                                   | 状态       |
| ---- | ----------------- | ------------------------------------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------------------ | ---------- |
| T01  | 2.1、8.2、8.3     | 最终 OCR 字段、模式 / 难度判定方式、支持的截图布局、各字段 ROI 坐标             | OCR PoC 阶段                   | 截图布局与归一化 ROI 配置、字段解析及谱面唯一判定规则                          | 开发时确定 |
| T02  | 7.1、8.4          | 曲名匹配通过阈值、候选差距阈值、业务校验条件、OCR 置信度用途                    | OCR PoC 与匹配调试阶段         | 基于真实样本的匹配参数和成功 / 失败判定规则；保持失败不入库                    | 开发时确定 |
| T03  | 14 阶段 4、15     | 标注样本覆盖范围、样本数量、字段 / 整条正确率及错误入库率的验收目标             | OCR PoC 阶段，批量体验优化前   | 固定样本集说明、量化目标及验证结果；以此定义“可接受水平”                       | 开发时确定 |
| T04  | 8、9、11.1        | 图片格式、分辨率、文件大小、批量数量、并发、页内取消 / 重试、进度与结果统计口径 | 上传与 OCR 队列开发阶段        | 输入限制、任务状态与交互说明；离开识别页即取消、截图不持久化的规则保持不变     | 开发时确定 |
| T06  | 6.2、10、12.2、14 | Song / Chart ID 生成规则、chartId 唯一范围、曲库更新及下架处理                  | 曲库与数据库开发阶段           | 稳定 ID 规范、唯一性约束及曲库更新规则；同一谱面仅一条成绩，保留已有成绩的关联 | 开发时确定 |
| T07  | 6.3、7            | Unicode 归一化形式、空格 / 标点 / 特殊符号处理清单、别名匹配细节                | 搜索模块开发阶段               | 统一归一化规则及代表曲名的匹配示例；原始展示文本保持独立                       | 开发时确定 |
| T08  | 10.1、11.3        | OCR 更新 / 手动编辑后的 source 语义、createdAt / updatedAt 更新时机             | 成绩写入与编辑开发阶段         | 各类写入操作的字段更新规则，包括成绩未变化时的处理                             | 开发时确定 |
| T09  | 3、8、15          | 浏览器 / 移动端支持范围、模型加载失败、数据库不可用或写入失败时的行为           | 环境验证及异常处理开发阶段     | 支持环境清单、错误提示和恢复方式；写入失败不得显示保存成功                     | 开发时确定 |
| T10  | 10.3、12.2        | Rating 规则 / 曲库版本号格式、全量重算触发时机、失败提示及恢复行为              | 版本管理与 Rating 重算开发阶段 | 版本规范与重算流程；任一版本变更必须重算存量 Rating，完成后才更新计算版本标记  | 开发时确定 |
