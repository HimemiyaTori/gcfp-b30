...
分数不高于已有最高分且未改变任何已存字段时，导入任务标记为“已跳过”，提示“重复成绩，已保留最高分”。新增成绩、提高分数或补充未知字段均标记为已处理，不显示该重复提示。
...
状态 `MISSION CLEAR`在读取曲名／分数前直接跳过，不新建、不更新任何成绩。FULL CHAIN → FC，ALL PERFECT → AP + FC，CLEAR / FAILED → 无 FC / AP。未知或不完整状态失败，不把它默认为 CLEAR。选曲页 0 分且无完成状态视为未游玩占位并跳过。
